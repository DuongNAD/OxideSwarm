...
) else if exist "target\release\rusty-grid.exe" (
    echo [INFO] Running native release binary [rusty-grid]...
    target\release\rusty-grid.exe agent --hub "%HUB_URL%" --id "%NODE_ID%" --platform windows
) else if exist "target\debug\rusty-grid.exe" (
    echo [INFO] Running native debug binary [rusty-grid]...
    target\debug\rusty-grid.exe agent --hub "%HUB_URL%" --id "%NODE_ID%" --platform windows
...
) else if exist "%~dp0..\..\target\release\rusty-grid.exe" (
    echo [INFO] Running native release binary [rusty-grid]...
    "%~dp0..\..\target\release\rusty-grid.exe" agent --hub "%HUB_URL%" --id "%NODE_ID%" --platform windows
) else if exist "%~dp0..\..\target\debug\rusty-grid.exe" (
    echo [INFO] Running native debug binary [rusty-grid]...
    "%~dp0..\..\target\debug\rusty-grid.exe" agent --hub "%HUB_URL%" --id "%NODE_ID%" --platform windows
) else (
